...
When you connect identity integrations such as [Google Workspace](../../integrations/google_workspace.mdx), [Azure Entra ID](../../integrations/azure_entra_id.mdx), or [Authentik](../../integrations/authentik.mdx), each sync pulls that directory's accounts into Openlane. The same person usually exists in several of these systems at once, so ingestion is followed by identity resolution: matching every synced account to a single [personnel record](./overview.mdx) (<Term t="identity-holder">identity holder</Term>), no matter how many directories they appear in.
...
Email matching considers every address on both sides: the account's primary address plus its aliases against each personnel record's email plus its aliases. A person whose GitHub account uses a personal alias still resolves to the same record as their Workspace account when either side knows about the shared address. Name matching links an account only when exactly one person matches the exact first and last name. When the name is ambiguous, such as two existing people named Jordan Smith, the account gets a new record you can merge deliberately rather than a link that may point at the wrong person.
...
## Primary Directory Field Authority
...
No, but without one, personnel records created by sync carry only names and emails; status, title, and department stay unset for you to manage manually. Designating a primary directory keeps status, title, and department current automatically.
