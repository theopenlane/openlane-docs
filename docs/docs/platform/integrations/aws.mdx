...
If your team runs infrastructure on AWS, this integration brings Security Hub <Term t="finding">findings</Term> and AWS Config assessment data into Openlane automatically. It uses a read-only, cross-account identity and access management (IAM) role, so Openlane reads your security data without touching your configuration.
...
- **Security Findings Ingestion:** Pulls Security Hub findings and normalizes them into Openlane vulnerability and finding records, giving you a single place to track remediation timelines and service level agreement (SLA) compliance.
- **AWS IAM:** Optionally read user access and groups and associate with <Term t="identity-holder">identity holders</Term> in Openlane.
...
2. Enter the following. The External ID is already pre-populated from Step 1, but confirm it matches if you have navigated away from the page:
...
:::warning
We recommend the cross-account role approach because static credentials do not rotate automatically and carry more risk. Use static credentials only when role assumption is not an option, scope the IAM user permissions as narrowly as possible, and rotate the key on a regular cadence.
:::
...
2. Enter the following fields, leaving `roleArn` and `externalId` empty, as they are not used with static credentials:
...
After saving, Openlane runs a health check against AWS and displays the result on the **Installed** tab of the Integrations page. You should see a **Healthy** badge confirming connectivity. If the badge shows **Needs Attention**, review the troubleshooting section below.
...
- Deduplicates by `externalID`, with Common Vulnerabilities and Exposures (CVE) fallback matching when present, so repeated scans update existing records instead of creating duplicates.
- Links each vulnerability to the AWS integration that produced it.
- Stores raw payload data

### What You Can Do Next

Once the findings land in Openlane, you can link them to affected assets, create remediation tasks, and track resolution against SLAs. During audits, this gives you a clear trail from finding to fix when an auditor asks for system monitoring or vulnerability management evidence.
