...
If your organization uses Tailscale to manage private network connectivity, this integration pulls device inventory and directory data into Openlane so you have visibility into who is on your tailnet and what devices are enrolled, supporting access control and endpoint <Term t="evidence">evidence</Term>.

## Key Capabilities

- **OAuth Authentication:** Connects using a scoped OAuth credential from the Tailscale admin panel, so there are no long-lived API keys to manage and access is scoped to specific read-only operations.
- **Directory Sync:** Reads users from your tailnet, giving you the identity baseline for access reviews and onboarding/offboarding evidence.
- **Asset Sync:** Ingests enrolled devices with route and posture attribute metadata, supporting device inventory and endpoint visibility.
...
After saving, Openlane runs a health check against the Tailscale API and displays the result on the **Installed** tab of the Integrations page. You should see a **Healthy** badge confirming connectivity. If the badge shows **Needs Attention**, review the troubleshooting section below.

## What Openlane Syncs

Openlane reads users, groups, and group memberships from your tailnet, along with every enrolled device and its metadata. All tailnet members are synced; groups and memberships are skipped when **Disable Group Sync** is enabled, which is useful if you only need user and device data.
