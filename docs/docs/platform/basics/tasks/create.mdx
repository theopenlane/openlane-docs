...
:::tip
    Refer to our documentation for a list of all fields supported on [controls](/docs/api/graph-api/objects#tasks). As of today, all fields should be supported. When putting fields that are lists in the csv, such as `Tags` they need to be in as JSON, for example `[""soc2"", ""evidence""]` with double quotes escaped.
:::
