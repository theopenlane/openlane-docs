...
Custom Data lets you shape how your compliance data is categorized without changing core workflows. Two complementary tools live under Organization Settings → Custom Data: **Custom Enums** define fixed dropdown values so records are classified consistently, and **Custom Tags** apply free-form labels for filtering and grouping.
...
Each enum value can include a name, an optional description, and a color; the color helps you distinguish values at a glance in dropdowns and reports.
...
A Custom Tag is a reusable label that can be applied to records such as controls, risks, tasks, policies, vendors, assets, and more. Reach for a tag instead of an enum when a classification is optional, spans multiple object types, or changes too often to maintain as a fixed value list.
...
| Use for filtering and grouping      | Use for reporting and governance    |
