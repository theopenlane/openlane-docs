...
Openlane supports **OpenID Connect (OIDC)** for single sign-on (SSO). You will need the following values from your identity provider (IdP) to configure SSO in Openlane.
...
The Authentication settings page surfaces the controls that determine how people sign in to your organization and how they become members.
...
:::info
**SSO exempt domains** are re-evaluated at each sign-in. If you remove a domain from the list, members on that domain are subject to SSO enforcement again the next time they sign in. A per-member exemption persists until you remove it from that member.
:::
...
<details>
<summary>What is OIDC and how is it different from Security Assertion Markup Language (SAML) or plain OAuth 2.0?</summary>
