...
We built CSV import so you can bring in controls you have already defined in other systems or spreadsheets, or pre-compose data before loading it into Openlane. Framework controls, custom controls, and control mappings can all come in this way.

## Importing Controls

Controls will generally fall into two buckets: those that are from a published framework or standard (ex: NIST 800-53), and custom controls. We recommend that you orient your <Term t="compliance-program">compliance program</Term> to be based on an existing framework / standard even if you may have some custom controls outside of that. This way, you can add implementation details, objectives, <Term t="evidence">evidence</Term>, etc., related to the controls from the published framework and receive the ongoing benefits of updates and changes we will publish as the upstream standard(s) are updated.

### Framework Controls

Even if you are coming with controls from another system, we recommend cloning them from our existing standard catalog rather than creating all of the controls as custom. To accomplish this, you'll need to ensure you have the fields outlined in the below table.

### Framework Control CSV Fields
...
| **Status**                 | The current status of the control.                                                                                                                          | Common values include `NOT_IMPLEMENTED`, `PREPARING`, `NEEDS_APPROVAL`, `APPROVED`, See all values in our [documentation](/graphql).                                                       | ❌ No     |
| **ReferenceID**            | Internal reference ID of the control object.                                                                                                                | Useful for linking to external systems                                                                                          | ❌ No     |
| **All control fields**     | All other fields on the [`control` object](/graphql) are supported except for a few list-type fields. | Refer to documentation for the complete list.                                                                                                  | ❌ No     |

:::tip
    Refer to our documentation for a list of all fields supported on [controls](/graphql). As of today, all fields should be supported. When putting fields that are lists in the csv, such as `MappedCategories` they need to be in as JSON, for example `["Security"]`. If the field is JSON, such as `ExampleEvidence` you need to escape the quotes, for example: `[{""documentationType"":""Other documentation"",""description"":""Evidence that the values have been communicated to employees during onboarding and annually thereafter""}]"`
:::

#### Framework Control CSV Example
...
The above will clone the `AC-1` control from `NIST 800-53`, set the status of the control to `NEEDS_APPROVAL` and the reference ID to `INT-0001`, and add implementation guidance to what is provided by the standard. It will additionally create a control implementation and comment linked back to the created control.

:::note
    The case of the field names matters. All field names _must_ be in `UpperCamelCase` and any field directly on the control object must be prefixed with `ControlInput`. For example `status` would be `ControlInput.Status`
:::

### Uploading Framework Controls

<Tabs groupId="console-vs-api" queryString>
...
1. Once you've selected your csv, click `Upload`. Depending on the number of controls this may take a little bit of time. If you have issues with timeouts, try breaking your csv into smaller chunks and uploading multiple times.
1. When it is complete, the dialog will close and you should see a success message; the table should now be populated with your controls
...
:::info
    You'll need an API token to follow these steps. You can create an api token in console in [developer settings](https://console.theopenlane.io/settings/developers)
:::

Using a GraphQL tool such as Apollo Explorer, you can send an API request:
...
1. Run the query. You should see the controls created and a response returned such as:
...
### Custom Control CSV Fields

All other fields on the [`control` object](/graphql) are supported. The `RefCode` is the only required field.

:::tip
    Refer to our documentation for a list of all fields supported on [controls](/graphql). As of today, all fields should be supported. When putting fields that are lists in the csv, such as `MappedCategories` they need to be in as JSON, for example `["Security"]`. If the field is JSON, such as `ExampleEvidence` you need to escape the quotes, for example: `[{""documentationType"":""Other documentation"",""description"":""Evidence that the values have been communicated to employees during onboarding and annually thereafter""}]"`
:::

#### Custom Control CSV Example
...
:::note
    The case of the field names matters. All field names _must_ be in `UpperCamelCase`
:::
...
1. You should now have a dialog where you can either use the file selector or drag and drop your csv
1. Once you've selected your csv, click `Upload`. Depending on the number of controls this may take a little bit of time
1. When it is complete, the dialog will close and you should see a success message; the table should now be populated with your controls
...
Prior to creating the mappings, you must ensure all the expected controls exist in your organization. For example, if you imported your custom controls, but are mapping them to System and Organization Controls 2 (SOC 2) controls, you must first add the SOC 2 controls to your organization. This can be done by adding them from the standards catalog or creating a SOC 2 program which will automatically add them to your organization.

### Mapping CSV Fields

The CSV can contain any of the fields from the [MappedControl](/graphql) object, again in the `UpperCamelCase`format. For example:
...
1. Click `Upload`
1. After successful upload, you should see the mappings on the `Linked Controls` panel on the control view screens

</Steps>

:::tip
    Refer to our documentation for a list of all [relationship types](/graphql) supported.
:::