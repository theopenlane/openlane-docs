...
Every major framework expects systematic risk management: a maintained register showing that you identify threats, prioritize by severity, and select controls based on actual risk. It answers the auditor's core question (how do you know your controls address your real threats?) and helps you decide where to spend security effort next.

## The Risk Lifecycle

Whatever vocabulary your framework uses, risk management follows the same loop:

1. **Identify.** Paint a complete picture of what could go wrong, through data analysis, stakeholder interviews, and review of external trends. The result is your risk register. Risks come from every direction (strategic, operational, financial, technology, compliance, and external events) and can threaten confidentiality, integrity, availability, privacy, reputation, or the balance sheet directly.
...
Status tracks where each risk stands: Open risks need attention, Mitigated risks have been reduced to acceptable levels (keep the controls monitored), Ongoing risks are under active management, and Archived risks are resolved or no longer relevant.
...
Risks don't live alone. Controls exist to mitigate specific risks; assessments should drive which controls you implement and prioritize, and control effectiveness feeds back into risk ratings. Action plans turn treatment decisions into concrete steps, and completing them moves a risk toward Mitigated. Risks can also be managed within compliance program contexts and associated with specific assets and entities, so accountability lands on the right component and the right owner.
...
Risks are not only hand-authored. Integrations can create or update risk records during ingestion (the [GCP Security Command Center](../../integrations/gcp_scc.mdx) integration does this alongside findings and vulnerabilities), with provenance details preserving where the risk came from and when the source last saw it.
...
No. The model works with COSO, FAIR, ISO 31000, NIST, or no named framework at all; impact, likelihood, and status are all you need to start. See [Risk Frameworks](./riskmanagement.mdx) if you want a methodology to lean on.
...
<details>
<summary>Can integrations create risks automatically?</summary>

Yes. Integration ingestion can create or update risk records, and the GCP Security Command Center integration does so alongside findings and vulnerabilities. Ingested risks keep an external identifier, the integration that surfaced them, and the time the source last observed them, so automated entries stay distinguishable from ones your team authored.
