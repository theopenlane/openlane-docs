...
1. **Enter your domain.** In the Domain settings, enter your desired subdomain (e.g., `trust.yourcompany.com`) and click **Set**. You should see the DNS records you need to add.

1. **Add a CNAME record** with your domain provider. It routes traffic to the Openlane infrastructure:
...
1. **Add a TXT record** to verify domain ownership. Use the exact values shown in the console:
...
1. **Verify.** Click **Verify** in the console. DNS changes typically propagate within 2-15 minutes, though it can take up to 72 hours in rare cases.
...
We provision SSL certificates automatically after DNS verification. Wait a few minutes for provisioning to complete.
