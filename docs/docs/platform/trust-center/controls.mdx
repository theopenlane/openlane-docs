...
The Controls page lists the available Trust Center controls grouped by domain (for example, **Governance & Policy**), and you can add or remove controls individually or a full domain at once. We recommend starting from the recommended set, which gives you a sensible baseline, and then working through one domain at a time so the published selection shows coherent coverage rather than a scattered list.
...
- Start from the recommended controls to get a sensible baseline, then refine
