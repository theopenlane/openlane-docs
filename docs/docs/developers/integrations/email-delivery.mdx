...
Openlane delivers email through the [`emailtemplates`](https://github.com/theopenlane/emailtemplates) library with asynchronous processing via the Riverboat job queue. Handlers render a templated message, enqueue it as a job, and dedicated River workers deliver it through the configured provider; nothing sends email inline on the request path.
...
Template rendering lives in the `emailtemplates` library; queueing, delivery, retry, and job-status tracking are handled by Riverboat workers. The sections below cover configuration, delivery behavior, and operational concerns.

## Email Template System

### Template Library Integration

We use the `github.com/theopenlane/emailtemplates` library for consistent, branded email communications:
...
Templates fall into two families: authentication flows (verification, password reset, login notifications) and business notifications (invitations, subscriber confirmations, system alerts). The [emailtemplates](https://github.com/theopenlane/emailtemplates) library holds the full set, so check it rather than this page when you need to confirm whether a template exists.
...
We use Riverboat (River) for asynchronous email processing:
...
| Concern | Mitigation |
|---|---|
| Sender spoofing | Sender Policy Framework and DomainKeys Identified Mail (SPF/DKIM) and Domain-based Message Authentication, Reporting, and Conformance (DMARC) records for email authentication |
| Transport interception | Encrypted SMTP connections (TLS/SSL) |
| Email abuse | Rate limiting on send paths |
| Cross-site scripting (XSS) in templates | Content sanitization |
| Stale action links | Token expiry on time-sensitive links |
| Personally identifiable information (PII) exposure | Careful handling of PII in email content and logs |
| Sensitive job payloads | Encryption of sensitive data in the job queue |
| Accountability | Audit logging of email delivery events |
| Data accumulation | Retention policies for email job data |
| EU recipients | General Data Protection Regulation (GDPR) compliance |
