...
| Email Type | Purpose | Category |
|---|---|---|
| Email Verification | New user account verification | Authentication |
| Password Reset | Secure password reset with tokens | Authentication |
| Password Reset Success | Confirmation of password changes | Authentication |
| Login Notifications | Security alerts for account access | Authentication |
| User Invitations | Organization and team invitations | Business |
| Subscriber Confirmations | Newsletter and update subscriptions | Business |
| Notification Emails | System alerts and updates | Business |
| Welcome Messages | Onboarding communications | Business |
...
| Concern | Mitigation |
|---|---|
| Sender spoofing | SPF/DKIM/DMARC records for email authentication |
| Transport interception | Encrypted SMTP connections (TLS/SSL) |
| Email abuse | Rate limiting on send paths |
| XSS in templates | Content sanitization |
| Stale action links | Token expiry on time-sensitive links |
| PII exposure | Careful handling of PII in email content and logs |
