...
- Push-style integrations use the same flow but skip per-integration secret credentials and recurring polling (this is basically only SCIM, but it is a good example of the built-in flexibility to accommodate a wide variety of capabilities in the same system)
