...
description: Architecture of building integrations within Openlane
sidebar_position: 1
keywords:
  - integration
  - builder
...
:::info
The canonical architecture doc lives beside the code at [`internal/integrations/README.md`](https://github.com/theopenlane/core/blob/main/internal/integrations/README.md); if this page and that README disagree, trust the README.
:::
...
- Push-style integrations use the same flow but skip per-integration secret credentials and recurring polling (this is basically only System for Cross-domain Identity Management (SCIM), but it is a good example of the built-in flexibility to accommodate a wide variety of capabilities in the same system)
...
1. Apply integration-level and definition-shipped Common Expression Language (CEL) filters
...
The tradeoff is that Go type names become part of the public interface. Renaming `GitHubAppToken` to `GitHubInstallationToken` is a migration rather than a refactor. We accept this because the alternative (invisible string drift between code and stored identifiers) is worse.
...
1. Choose the credential-slot name first, since it becomes part of the public interface
...
Use CEL for filtering and projection rather than heavy normalization. If the expression starts encoding provider-specific control flow, that logic belongs in provider code instead.
...
:::warning
These invariants must hold for a definition to be valid:

- The selecting `credentialRef` must belong to the connection that uses it, and auth and disconnect must refer only to slots in that same connection
...
- A credential slot with no user-facing schema is meaningful rather than incomplete. Auth-managed slots work this way because the browser or app-install flow produces the credential, so there's nothing to render in a manual form
...
#### Metadata and Connection Lifecycle
...
"Stock persistence" means a generated starting point rather than a final implementation. Straightforward upserts may work unchanged; anything that needs extra lookup resolution, edge resolution, or conflict handling usually needs custom persistence logic.
