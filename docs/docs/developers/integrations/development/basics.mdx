...
Building a new integration means implementing a provider: its config, credential exchange, health checks, and operations. Directory Sync integrations have [additional requirements](./directory-sync.mdx).
...
`UserInput` holds per-installation config that the end user provides during setup (e.g. filter expressions, flags). If an integration has multiple operations, e.g. `DirectorySync` and `FindingSync`, each operation should have its own config within `UserInput`. The individual per-sync configs are what are passed to the `OperationSchema`. This will allow for configuration of each operation individually, for example:
...
`Clients` lists the clients the definition can build for the integration. Some integrations, such as the `AWS` integration, will have multiple clients for different operations, while others will only have a single client definition.
...
Every integration needs at least a health check operation. It runs immediately after connection to confirm the integration is set up correctly.
...
Each additional operation performed by an integration follows the same pattern as the health check, but defines how the data is pulled and which schemas the data is ingested into within Openlane. For example, a `directory_sync` operation will almost always have an ingest schema that includes DirectoryAccounts, DirectoryGroups, and DirectoryMemberships. See [directory sync](./directory-sync.mdx) for more specific details on directory sync operations.
...
Common Expression Language (CEL) is used by ingest operations to filter and map provider payloads onto normalized schema keys. Any integration that uses `IngestHandle` will define CEL expressions.
