...
On top of the [core integration builder](./basics.mdx), additional pieces are required for integrations that implement **Directory Sync**, the process of collecting accounts, groups, and memberships from a provider and emitting them through the ingest pipeline.
...
Directory Sync integrations can include fields on the `UserInput` config:
...
- **`FilterExpr`**: a Common Expression Language (CEL) expression evaluated against each envelope before ingest, allowing users to ingest only relevant data; see [CEL Expressions](./basics.mdx#cel-expressions)
...
In `builder.go`, register the directory sync operation alongside the health check. Note `Reconcile: true`. Directory sync runs on a schedule rather than inline during connection.
