...
This guide covers what's additionally required on top of the [core integration builder](./building-an-integration.mdx) for integrations that implement **Directory Sync** — the process of collecting accounts, groups, and memberships from a provider and emitting them through the ingest pipeline.
...
- **`FilterExpr`** — a CEL expression evaluated against each envelope before ingest; see [CEL Expressions](./building-an-integration.mdx#cel-expressions)
