...
:::info
Entitlements are for the Openlane SaaS product. Running the server **without** the entitlements option enabled (the default for open-source deployments) means no feature gating at all: every feature of the platform is accessible.
:::

We integrate with Stripe to provide module-based billing and subscription management. The billing system manages organization subscriptions, feature entitlements, and usage-based pricing through a combination of Stripe APIs, OpenFGA authorization, and Redis caching.
...
|---|---|---|
| Base Module | Free core system access (included with all subscriptions) | Free |
| Compliance Module | Core compliance automation and standards library | $450 |
| Trust Center Module | Public security portal | $300 |
| Entity Management Module | Vendor risk management | $200 |
| Vulnerability Management Module | Vulnerability tracking | $100 |

### Available Add-ons

| Add-on | Description | Monthly Price |
|---|---|---|
| Domain Scanning Add-on | Domain and vulnerability scanning | $100 |
| Extra Evidence Storage | Additional 100GB storage | $10 |
| Policy Management Add-on | Policy and procedure management | $10 |
| Risk Management Add-on | Enhanced risk management | $10 |

:::info
Prices here reflect the catalog at the time of writing. The authoritative source is `pkg/catalog/catalog.yaml`, which is synchronized to Stripe on startup; check it before relying on these numbers.
:::
...
#### StripeClient
The [`pkg/entitlements`](https://github.com/theopenlane/core/blob/main/pkg/entitlements/README.md) package provides a wrapper around the official Stripe Go client (the [catalog README](https://github.com/theopenlane/core/blob/main/pkg/catalog/README.md) and [catalog CLI](https://github.com/theopenlane/core/blob/main/cmd/catalog/README.md) cover the module catalog and seeding):
