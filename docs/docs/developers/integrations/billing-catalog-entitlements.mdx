...
Openlane uses a two-tier billing structure. Modules deliver core platform functionality, such as `compliance_module` and `trust_center_module`, and every subscription requires at least one. Add-ons are optional enhancements that attach to an existing subscription, such as `domain_scanning_addon` and `extra_evidence_storage_addon`.

The full set of modules, add-ons, and their prices is defined in `pkg/catalog/catalog.yaml`, which is synchronized to Stripe on startup. Treat that file as the source of truth; prices copied into documentation drift.
