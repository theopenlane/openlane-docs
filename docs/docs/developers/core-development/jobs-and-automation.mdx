...
1. **Scheduled Jobs** are instances created from templates that:
...
1. **Job Runners** are execution environments that:
...
1. **Job Results** capture execution outcomes:
...
1. If Windmill integration is enabled, creates/updates a corresponding Windmill flow
1. The flow path is stored on the template for future reference
...
1. From `core-startup` run the following to seed some test job templates and scheduled jobs:
    ```bash
    task setup
