...
The core codebase has several non-obvious behaviors that commonly break first PRs: soft deletes, ORM-layer authorization, context propagation, and code generation ordering. Read this before your first PR, because the failure modes below produce errors that do not point back to their cause.

## IDE Build Tags

The core codebase uses build tags extensively for conditional compilation. Without `cli,test,codegen` in your IDE's Go build tags setting, functions gated behind `//go:build cli` or `//go:build test` will appear undefined, and your language server will flag imports as unused. See [Local Setup](../getting-started/local-setup.mdx#ide-build-tags) for the VSCode and GoLand configuration.
...
|---|---|
| Authenticated user | Privacy rules deny access, fine-grained authorization (FGA) checks fail |
...
This creates a circular dependency when adding a new schema: the schema references generated types that do not exist until after the first generation run. Work through it in stages:

<Steps>

1. Start your schema with the basic definitions and fields, but don't add Annotations or Policies, and in the mixin config use the local type on first generation (e.g. use `ActionPlan` instead of `generated.ActionPlan` in the above example) and also keep mixins and general config as light as possible
1. Once you've defined the basics, stage (or commit) your schema in your local branch - this step alone could save you a lot of headache
1. Run `task regenerate` for a fully clean run; resolve any problems that may arise during this process by first discarding any generated files or changes in your branch (since you've already committed or staged the schema), making the update to your schema, re-staging / committing those changes, and re-run. Do this repeatedly / rinse and repeat until you've gotten a clean run
1. A clean run with your base schema gets you the generated types; you can then go back to your schema and update the local types to the generated ones, and then add schema Annotations, policy, or other configurations

</Steps>

## CSRF Tokens on All Mutating Requests

The server enforces cross-site request forgery (CSRF) protection on `POST`, `PUT`, `PATCH`, and `DELETE` requests. If you are testing via `curl` or a REST client:
