...
This page illustrates the major authentication paths supported by Openlane (credential, OAuth, passkey, and organization-enforced SSO), along with the WebFinger discovery endpoint and the SSO enforcement and provisioning behavior behind them. Use it when you need to trace how a login request moves through the handlers.
...
- TFA is independent: `TFARequired` mirrors `multifactor_auth_enforced` regardless of SSO exemption.
...
The callback also marks the IdP-verified email confirmed (via `setEmailConfirmed`) before session generation. That confirmation triggers the email-confirmation hook, which drives domain auto-join for **non-enforced** organizations (see below); the enforced target org is joined by JIT, not auto-join.
