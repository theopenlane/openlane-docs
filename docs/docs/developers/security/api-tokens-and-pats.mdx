...
- **Scopes**: Array of per-object permission scopes in `object:action` format (e.g. `evidence:read`, `evidence:write`, `control:delete`). Most object type support `read`, `write`, and `delete` actions independently
...
- **Per-Object Scopes**: Permissions are granted per object type using `object:action` format — grant only the specific read, write, or delete actions each token requires
- **Scope Limitation**: Restrict token permissions to the minimum required access; avoid SELECT ALL for production tokens
