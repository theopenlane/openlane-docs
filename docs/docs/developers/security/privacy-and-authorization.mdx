...
- **Privacy policies** on Ent schemas that gate every query and mutation
- **OpenFGA tuples** that model relationships between users and resources
- **Hooks** that create, update, and delete tuples when data changes

## The Privacy Policy Sandwich

Every entity's privacy policy is assembled from three layers that execute in order: pre-policy rules, the entity's custom rules, and post-policy defaults. The custom rules sit between the two standard layers, which is where the name comes from.
...
Entity-specific rules defined in each schema's `Policy()` method. This is where fine-grained authorization (FGA) checks happen:
...
This layer is the failsafe. If no custom rule returned a decision:
...
The first rule to return `Allow` or `Deny` wins. A rule returns `Skip` when it does not have enough information to decide, and evaluation passes to the next rule.
...
- **Creator gets admin**: `user:creator-id → admin → entity:new-id`
- **Org gets parent**: `organization:org-id → parent → entity:new-id`
...
FGA checks involve network round-trips. To reduce latency, we cache positive authorization decisions in Redis via `pkg/permissioncache/`:
...
- **Check the privacy policy** on the entity schema: does it have the right mutation rules?
- **Check the user's context**: is the organization ID set? Is the user authenticated?
- **Check FGA tuples**: does a tuple exist granting the user access to the specific object?
- **Check module gating**: does the organization have the required feature modules?
- **Check the permission cache**: a recently-revoked permission may still be cached

:::tip
For local development, you can inspect FGA tuples directly using the OpenFGA CLI or the built-in FGA playground, started with `task docker:fga:up`; see [Run the Stack](../getting-started/run-the-stack.mdx). The FGA module is defined in `fga/model/fga.mod` in the core repository with corresponding model directories.
:::
