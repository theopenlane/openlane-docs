...
| 7 | **Authentication** | Validates JWT, personal access token (PAT), or API token from `Authorization` header |
| 8 | **System Admin Context** | Sets system admin flag in context if applicable |
| 9 | **Additional** | Cross-site request forgery (CSRF) validation, rate limiting, and other per-deployment middleware |
...
- **Start**: `ent.Client.Tx(ctx)` opens a transaction before the handler runs
- **Commit**: if the handler returns `nil`, the transaction commits
- **Rollback**: if the handler returns an error, the transaction rolls back
...
REST handlers live in `internal/httpserve/handlers/` and are registered via declarative configuration in `internal/httpserve/route/`. As a rule of thumb, you should not be writing REST handlers. We elected to implement both REST and GraphQL because REST is far more appropriate for purposes like authentication flows given the nature of the technology, but we want our user-facing APIs and business-driven logic to be built using GraphQL. When in doubt, reach out to us.
...
This means the OpenAPI documentation is always in sync with the actual handler signatures, and no manual spec maintenance is required.
